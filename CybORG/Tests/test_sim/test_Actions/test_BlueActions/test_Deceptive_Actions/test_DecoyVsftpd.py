...
def test_DecoyVsftpd_without_red(cyborg, params, obs_failure, host,blue_spam):
    # Here we test out the action without any red activity
    history = blue_spam(action=DecoyVsftpd, params=params,cyborg=cyborg)['history']
    ...
    assert connection['Service Name'] == 'vsftpd'
    assert connection['Username'] == 'ubuntu'
    assert connection['Properties'] == ['rfi']

@pytest.mark.skip
@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoyVsftpd_repeat(red_killchain, params, host, obs_failure,cyborg,blue_spam):
    # Here we test the actions work twice
    blue_actions = blue_spam(action=DecoyVsftpd, params=params,cyborg=cyborg)
    expected_obs = [x[1] for x in blue_actions['history'] if x[0] == host][0]
    cyborg = blue_actions['cyborg']
    action = DecoyVsftpd(hostname=host,**params)
...
def test_DecoyVsftpd_killchain(blue_spam,host,cyborg,red_killchain, params, exploit):
    ...
    assert results.observation[host]['Processes'][0]['Service Name'] == 'vsftpd'
...
@pytest.mark.skip
@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoyVsftpd_followup(red_killchain, params, host, cyborg,blue_spam, obs_failure):
