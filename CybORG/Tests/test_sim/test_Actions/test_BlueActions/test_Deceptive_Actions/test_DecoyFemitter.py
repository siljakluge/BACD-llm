...
def test_DecoyFemitter_without_red(cyborg, params, obs_failure, host,blue_spam):
    # Here we test out the action without any red activity
    history = blue_spam(action=DecoyFemitter, params=params,cyborg=cyborg)['history']
    ...
    assert connection['Service Name'] == 'femitter'
    assert connection['Username'] == 'SYSTEM'

@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoyFemitter_repeat(red_killchain, params, host, obs_failure,cyborg,blue_spam):
    # Here we test the actions work twice
    blue_actions = blue_spam(action=DecoyFemitter, params=params,cyborg=cyborg)
    expected_obs = [x[1] for x in blue_actions['history'] if x[0] == host][0]
    cyborg = blue_actions['cyborg']
    action = DecoyFemitter(hostname=host,**params)
    results = cyborg.step(action=action,agent='Blue')
    assert results.observation == obs_failure #expected_obs

@pytest.mark.parametrize('host', HOSTNAMES)
@pytest.mark.parametrize('exploit',[FTPDirectoryTraversal])
def test_DecoyFemitter_killchain(blue_spam,host,cyborg,red_killchain, params, exploit):
    ...
    assert results.observation[host]['Processes'][0]['Service Name'] == 'femitter'
...
@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoyFemitter_followup(red_killchain, params, host, cyborg,blue_spam, obs_failure):
