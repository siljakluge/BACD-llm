...
from CybORG.Shared.Actions import DecoySSHD , SSHBruteForce
...
def test_DecoySSHD_without_red(cyborg, params, obs_failure, host,blue_spam):
    # Here we test out the action without any red activity
    history = blue_spam(action=DecoySSHD, params=params,cyborg=cyborg)['history']
    ...
    assert connection['Service Name'] == 'sshd'
    assert connection['Username'] == 'ubuntu'

@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoySSHD_repeat(red_killchain, params, host, obs_failure,cyborg,blue_spam):
    # Here we test the actions work twice
    blue_actions = blue_spam(action=DecoySSHD, params=params,cyborg=cyborg)
    expected_obs = [x[1] for x in blue_actions['history'] if x[0] == host][0]
    cyborg = blue_actions['cyborg']
    action = DecoySSHD(hostname=host,**params)
    results = cyborg.step(action=action,agent='Blue')
    assert results.observation == obs_failure #expected_obs

@pytest.mark.parametrize('host',HOSTNAMES)
@pytest.mark.parametrize('exploit',[SSHBruteForce])
def test_DecoySSHD_killchain(blue_spam,host,cyborg,red_killchain, params, exploit):
    ...
    assert results.observation[host]['Processes'][0]['Service Name'] == 'sshd'
...
@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoySSHD_followup(red_killchain, params, host, cyborg,blue_spam, obs_failure):
