...
from CybORG.Shared.Actions import DecoySmss, EternalBlue
...
invalid_hosts = ['User2','User3','User4','Enterprise0','Enterprise1','Enterprise2',
        'Op_Server0','Op_Host0', 'Op_Host1', 'Op_Host2'] # Linux hosts and those with smss (User0, User1)

@pytest.mark.parametrize('parameter', ['hostname','session','agent'])
def test_DecoySmss_junk_input(params,parameter,cyborg):
    # Here we make sure the action handles junk inputs properly.
    params['hostname'] = 'User0'
    params[parameter] = 'Junk'
    action = DecoySmss(**params)
...
def test_DecoySmss_without_red(cyborg, params, obs_failure, host,blue_spam):
    # Here we test out the action without any red activity
    history = blue_spam(action=DecoySmss, params=params,cyborg=cyborg)['history']
    ...
    assert connection['Service Name'] == 'smss'
    assert connection['Username'] == 'ubuntu'

@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoySmss_repeat(red_killchain, params, host, obs_failure,cyborg,blue_spam):
    # Here we test the actions work twice
    blue_actions = blue_spam(action=DecoySmss, params=params,cyborg=cyborg)
    expected_obs = [x[1] for x in blue_actions['history'] if x[0] == host][0]
    cyborg = blue_actions['cyborg']
    action = DecoySmss(hostname=host,**params)
    results = cyborg.step(action=action,agent='Blue')
    assert results.observation == obs_failure #expected_obs

@pytest.mark.parametrize('host',HOSTNAMES)
@pytest.mark.parametrize('exploit',[EternalBlue])
def test_DecoySmss_killchain(blue_spam,host,cyborg,red_killchain, params, exploit):
    ...
    assert results.observation[host]['Processes'][0]['Service Name'] == 'smss'
...
@pytest.mark.parametrize('host', HOSTNAMES)
def test_DecoySmss_followup(red_killchain, params, host, cyborg,blue_spam, obs_failure):
